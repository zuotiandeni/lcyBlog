...
<template>
    <div>
        ...
        <div class="knowledge-block">
            <h3>2、闭包</h3>
            <div class="knowledge-detail">
                无论通过何种手段将内部函数传递到所在的<strong>词法作用域以外</strong>，它都会持有对原始定义作用 域的引用，无论在何处执行这个函数都会使用闭包。
            </div>
        </div>
        <div class="knowledge-block">
            <h3>3、reactive()</h3>
            <div class="knowledge-detail">
                1、仅对对象类型有效（对象、数组和 Map、Set 这样的集合类型），而对 string、number 和boolean这样的 原始类型 无效。
                <br/>
                2、因为 Vue的响应式系统是通过属性访问进行追踪的，因此我们必须始终保持对该响应式对象的相同引用。这意味着我们不可以随意地“替换”一个响应式对象，因为这将导致对初始引用的响应性连接丢失。
                <br/>
